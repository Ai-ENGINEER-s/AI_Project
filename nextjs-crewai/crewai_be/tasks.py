...
class CompanyResearchTasks():
    ...
    def manage_research(self, agent: Agent, companies: list[str], positions: list[str], tasks: list[Task]):
        """
        Manage Research Task:
        - Description: Use the results from the Company Research Agent to research each position in each company
          and put together a JSON object containing the URLs for 3 blog articles and the URLs and title 
          for 3 YouTube interviews for each position in each company.
        - Responsibilities:
            - Utilize the Company Research Agent to gather information.
            - Research each position in each company.
            - Compile the collected information into a JSON object.
        """
        return Task(
    ...
    def company_research(self, agent: Agent, company: str, positions: list[str]):
        ...
        - Description: Research the specified positions for the given company and find URLs for 3 recent blog articles 
          and the URLs and titles for 3 recent YouTube interviews for each person in each position.
        - Responsibilities:
            - Research each specified position for the company.
            - Find URLs for blog articles and YouTube interviews for each position.
